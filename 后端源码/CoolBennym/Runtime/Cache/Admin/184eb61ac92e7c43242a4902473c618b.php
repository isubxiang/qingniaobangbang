...
<div class="main-tu-js main-sc">
    ...
    <div class="tudou-js-nr">
        <div class="tu-select-nr" style="margin-top: 0px; border-top:none;">
            ...
            <div class="right">
                <form class="search_form" method="post" action="<?php echo U('user/recycle');?>">
                    <div class="seleHidden" id="seleHidden">
                        <label>
                            <span>账户</span>
                            <input type="text" name="account" value="<?php echo ($account); ?>" class="tu-inpt-text" />
                        </label>
                        
                          <label>
                            <span>手机号码</span>
                            <input type="text" name="mobile" value="<?php echo ($mobile); ?>" class="tu-inpt-text" />
                        </label>
                        <label>
                            <span>昵称</span>
                            <input type="text" name="nickname" value="<?php echo ($nickname); ?>" class="tu-inpt-text" />
    ...
    <form  target="x-frame" method="post">
        <div class="tu-table-box">
            <table bordercolor="#dbdbdb" cellspacing="0" width="100%" border="1px"  style=" border-collapse: collapse; margin:0px; vertical-align:middle; background-color:#FFF;"  >
                <tr>
                    ...
                    <td>余额</td>
                    <td>积分</td>
                    <td><?php echo (($CONFIG["prestige"]["name"])?($CONFIG["prestige"]["name"]):'威望'); ?></td>
                    <td>冻结金</td>
                    <td>注册时间</td>
                    <td class="w150">操作</td>
                </tr>
                <?php if(is_array($list)): foreach($list as $key=>$var): ?><tr>
                        ...
                        <td>商户冻结金：&yen; <?php echo round($var['frozen_gold']/100,2);?>元</eq>会员冻结金&yen; <?php echo round($var['frozen_money']/100,2);?>元</td>
                        <td><?php echo (date('Y-m-d H:i:s',$var["reg_time"])); ?></td>
                        <td>
                        	<?php echo BA('user/renew',array("user_id"=>$var["user_id"]),'恢复','act','tu-dou-btn');?>
                        	<?php echo BA('user/recycle_delete',array("user_id"=>$var["user_id"]),'彻底删除','act','tu-dou-btn');?>
                        </td>
        ...
        <div class="tu-select-nr" style="margin-bottom: 0px; border-bottom: none;">
            <div class="left">
            	<?php echo BA('user/renew','','批量恢复','list',' tu-dou-btn');?>
            </div>
        </div>
    </form>
</div>
