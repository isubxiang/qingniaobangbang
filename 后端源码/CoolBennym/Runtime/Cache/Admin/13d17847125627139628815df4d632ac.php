...
<form target="x-frame" action="<?php echo U('adsite/edit',array('site_id'=>$detail['site_id']));?>" method="post">
                ...
                <tr>
                    ...
                    <td  class="tu-right-td">
						<select name="data[theme]">
							<?php if(is_array($template)): foreach($template as $key=>$item): ?><option value="<?php echo ($item[theme]); ?>"  <?php if(($item[theme]) == $detail["theme"]): ?>selected="selected"<?php endif; ?> ><?php echo ($item[name]); ?></option><?php endforeach; endif; ?>
						</select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td class="tu-right-td">
                        <select name="data[site_type]" class="tu-manage-select w300">
                            <?php if(is_array($types)): foreach($types as $key=>$var): ?><option value="<?php echo ($key); ?>"  <?php if(($key) == $detail["site_type"]): ?>selected="selected"<?php endif; ?> ><?php echo ($var); ?></option><?php endforeach; endif; ?>
                        </select> 
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td  class="tu-right-td">
                        <select name="data[site_place]" class="tu-manage-select w300">
                            <?php if(is_array($place)): foreach($place as $key=>$var): ?><option value="<?php echo ($key); ?>"  <?php if(($key) == $detail["site_place"]): ?>selected="selected"<?php endif; ?> ><?php echo ($var); ?></option><?php endforeach; endif; ?>
                        </select>
                        <code>一旦确定好，请不要随意编辑</code>
                    </td>
                </tr>
