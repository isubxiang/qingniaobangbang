...
class RunningAction extends CommonAction{
    ...
    public function detail($running_id = 0){
	 ...
	 public function deliveryPublish(){
		...
		if($this->isPost()){
			...
			$data = $this->checkFields($this->_post('data',false),array('delivery_id','user_id','SchoolId','school_id','StudentCode','RealName','Major','IdCode','Gender','EnrollmentDate','Department','PicUrl0','PicUrl1','phoneNumber','audit'));
	...
	//操作学校
	 public function schoolPublish($school_id = 0){
		...
		if($this->isPost()){
			...
			$data = $this->checkFields($this->_post('data',false),array('user_id','city_id','school_id','area_id','business_id','Id','Name','Region','FreightMoneyCaption','MinFreightMoney','is_cash','user','user_big','user_cash_commission','shop','shop_big','shop_cash_commission','user_cash_second','shop_cash_second','admin_yongjin_rate','city_yongjin_rate','lat','lng','orderby'));
	...
	public function refund($running_id = 0,$p= 0){
		...
		//原路退回资金
		$info = '管理员强制退款跑腿订单ID【'.$v['running_id'].'】';
		$mix = $this->config['running']['running_weixin_original_refund_mix'] ? $this->config['running']['running_weixin_original_refund_mix'] : 10;
		$mix2 = $mix*100;
		if($this->config['running']['running_weixin_original_refund'] == 1 && $v['MoneyPayment'] < $mix2  && $v['MoneyTip'] == 0){
			$runningOrderRefundUser = D('Running')->runningOrderRefundUser($v['running_id'],$v['user_id'],$v['MoneyPayment'],'running',$info);
