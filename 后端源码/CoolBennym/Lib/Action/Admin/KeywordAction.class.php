...
class KeywordAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            var_dump($data);
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('keyword/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
    ...
    public function edit($key_id = 0)
    {
        if ($key_id = (int) $key_id) {
            ...
            if (!($detail = $obj->find($key_id))) {
                $this->tuError('请选择要编辑的关键字');
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
                $data['key_id'] = $key_id;
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('keyword/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($key_id = 0)
    {
        if (is_numeric($key_id) && ($key_id = (int) $key_id)) {
            $obj = D('Keyword');
            $obj->delete($key_id);
            $this->tuSuccess('删除成功', U('keyword/index'));
        } else {
