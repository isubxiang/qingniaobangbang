...
class EmailAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            $obj = D('Email');
            if ($obj->add($data)) {
                $this->tuSuccess('添加成功', U('email/index'));
            }
            $obj->cleanCache();
            $this->tuError('操作失败');
        } else {
    ...
    public function edit($email_id = 0)
    {
        if ($email_id = (int) $email_id) {
            ...
            if (!($detail = $obj->find($email_id))) {
                $this->tuError('请选择要编辑的邮件模版');
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
                $data['email_id'] = $email_id;
                if (false !== $obj->save($data)) {
                    $this->tuSuccess('操作成功', U('email/index'));
                }
                $obj->cleanCache();
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($email_id = 0)
    {
        if (is_numeric($email_id) && ($email_id = (int) $email_id)) {
            ...
            $obj->cleanCache();
            $this->tuSuccess('关闭成功', U('email/index'));
        } else {
    ...
    public function audit($email_id = 0)
    {
        if (is_numeric($email_id) && ($email_id = (int) $email_id)) {
            ...
            $obj->cleanCache();
            $this->tuSuccess('开启成功', U('email/index'));
        } else {
            $email_id = $this->_post('email_id', false);
            if (is_array($email_id)) {
                $obj = D('Email');
