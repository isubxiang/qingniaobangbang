...
class ThreadAction extends CommonAction{
	...
	public function child($cate_id = 0){
        $datas = M('thread')->where(array('cate_id'=>$cate_id))->select();
        $str = '<option value="0">=请选择所属贴吧=</option>' . "\n\r";
        foreach($datas as $var){
             $str .= '<option value="' . $var['thread_id'] . '">' . $var['thread_name'] . '</option>' . "\n\r";
        }
        echo $str;
        die;
    }
    ...
    private function createCheck(){
        $data = $this->checkFields($this->_post('data', false), array('school_id','thread_name','cate_id','user_id','intro', 'photo','banner','is_hot','is_essence','orderby'));
