	...
	public function setting($cate_id){
        ...
        if($this->isPost()){
            ...
            foreach($old as $key => $val){
                D('RunningCateAttr')->save(
				array(
					'attr_id' => (int) $key, 
					'attr_name' => htmlspecialchars($val['attr_name']), 
					'attr_money' => $val['attr_money'], 
            ...
            $this->assign('detail', $detail);
            $this->assign('attrs', $attrs = D('RunningCateAttr')->order(array('orderby' =>'asc'))->where(array('cate_id' => $cate_id))->select());
            $this->display();
        }
    }
