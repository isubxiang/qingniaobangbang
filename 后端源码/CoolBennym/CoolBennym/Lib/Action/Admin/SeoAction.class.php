...
class SeoAction extends CommonAction
{
    ...
    public function create()
    {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->tuSuccess('添加成功', U('seo/index'));
            }
            $this->tuError('操作失败');
        } else {
            $this->display();
        ...
        if ($seo_id = (int) $seo_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->tuSuccess('操作成功', U('seo/index'));
                }
                $this->tuError('操作失败');
            } else {
    ...
    public function delete($seo_id = 0)
    {
        if (is_numeric($seo_id) && ($seo_id = (int) $seo_id)) {
            ...
            $obj->delete($seo_id);
            $obj->cleanCache();
            $this->tuSuccess('删除成功', U('seo/index'));
        } else {
